...
(function ($) {
  var ColorPicker = function () {
    ...
    <div class="colorpicker_rgb_a colorpicker_field">
        <input type="text" maxlength="3" size="3" />
        <span></span>
    </div>
...
`;
      ...
      fillHexFields = function (hsb, cal) {
        $(cal).data('colorpicker').fields
          .eq(7).val(HSBToHex(hsb)).end();
      },
      setSelector = function (hsb, cal) {
        $(cal).data('colorpicker').selector.css('backgroundColor', '#' + HSBToHex({h: hsb.h, s: 100, b: 100, a: 255}));
      ...
      setAlpha = function (rgb, cal) {
        $(cal).data('colorpicker').alpha.css('left', parseInt(150 - 150 * rgb.a/255, 10));
      },
      ...
      change = function (ev) {
        ...
        if (this.parentNode.className.indexOf('_hex') > 0) {
          ...
          cal.data('colorpicker').color = col = fixHSB({
            ...
            a: parseInt(cal.data('colorpicker').fields.eq(3).val(), 10)
          });
        } else {
      ...
      moveAlpha = function (ev) {
        change.apply(
          ev.data.cal.data('colorpicker')
            .fields
            .eq(3)
            .val(parseInt(255*(150 - Math.max(0,Math.min(150,(ev.pageX - ev.data.x))))/150, 10))
            .get(0),
          [ev.data.preview]
        );
        return false;
      ...
      show = function (ev) {
        ...
        var left = pos.left;
        if (top + pickerHeight > viewPort.t + viewPort.h) {
          top -= this.offsetHeight + pickerHeight;
        }
        if (left + pickerWidth > viewPort.l + viewPort.w) {
          left -= pickerWidth;
        }
      ...
      HexToRGB = function (hex) {
        ...
        return {r: (hex & 0xFF000000) >>> 24, g: (hex & 0x00FF0000) >>> 16, b: (hex & 0x0000FF00) >>> 8, a: (hex & 0x000000FF)};
      },
      ...
      HSBToRGB = function (hsb) {
        ...
        if(s == 0) {
          ...
          else {rgb.r=0; rgb.g=0; rgb.b=0, rgb.a=255}
        }
        rgb.a = hsb.a;
